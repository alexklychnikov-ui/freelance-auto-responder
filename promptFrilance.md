...
- Python, боты, парсинг, API, AI/LLM, RAG, автоматизация, FastAPI, интеграции, веб-MVP
- OCR / распознавание текста (PDF, сканы, фото), email-автоматизация (IMAP / Gmail / почта, вложения) — кейс TGRecordsOfExpenses
- Лендинг, одностраничный сайт, сайт-визитка на Next.js/React/Tailwind — кейс MyPortfolio
...
- Чистый дизайн/макет БЕЗ разработки (только Figma/PSD)
- 1С, мобильная нативная разработка без Python
...
ВАЖНО: OCR/email на Python — Автоматизация/интеграция, НЕ «чистый дизайн» и НЕ «нет опыта».
Если в github_stack/lightrag есть OCR (TGRecordsOfExpenses) и ТЗ про OCR/документы/почту — score >= 7, fit=true.
matched_skills примеры: Python, OCR, IMAP/email.
ВАЖНО: «одностраничный сайт / лендинг по ТЗ» на Next.js — веб-MVP, не «чистый дизайн».
Если в контексте есть MyPortfolio — score >= 7 для таких заказов.
